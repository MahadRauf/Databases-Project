...
<body>
	...
	try {
		...
		String select = "SELECT username, type FROM account WHERE username = ? AND type = 2";
		PreparedStatement pd = con.prepareStatement(select);
		pd.setString(1, username);
		ResultSet rs = pd.executeQuery();
		if(!rs.next()){
			response.sendRedirect("repDelete.jsp");
		}
		...
		String delete = "DELETE FROM account WHERE username = ? AND type = 2";
		...
		con.close();
		out.print("Account succesfully deleted.");
